...
Component({
  ...
  observers:{
    'cameraShow':async function(cameraShow){
      ...
      that.data.canLocation && hasPermission && await that.getPermission(that,'userLocation')
      .then(r => {
        // 获取相机权限结果
      ...
      if(cameraShow && hasPermission && hasLocationPermission || 
        !that.data.canLocation && cameraShow && hasPermission
        ){
        // 开启定时器
  ...
  methods: {
    ...
    takePhoto: function (params) {
      ...
      let { brforeCreate,success,complete,canLocation,canTime,canProjectName } = params
      ...
      this.setData({
        ...
        canLocation:canLocation || false,
        canProjectName:canProjectName || false,
        canTime:canTime || false
      })
    },
          ...
          wx.getImageInfo({
            ...
            success: function (res) {
              ...
              if(that.data.portrait) {
                ...
                that.data.canLocation && that.canvas.fillText(`拍摄地址：${that.data.district}`, 20, that.data.height * .8 - 70 + 15)
                that.data.canTime && that.canvas.fillText(`拍摄时间：${that.data.currentTime}`, 20, that.data.height * .8 - 70 + 35)
                that.data.canProjectName &&that.canvas.fillText(`项目名称：${that.data.projectName}`, 20, that.data.height * .8 - 70 + 55)
                ...
                if(!that.data.Gamma){
                  ...
                  that.data.canLocation && that.canvas.fillText(`拍摄地址：${that.data.district}`, 20, -50)
                  that.data.canTime && that.canvas.fillText(`拍摄时间：${that.data.currentTime}`, 20, -30)
                  that.data.canProjectName && that.canvas.fillText(`项目名称：${that.data.projectName}`, 20, -10)
                }else{
                  that.canvas.setFillStyle('rgba(0, 0, 0,.3)')
                  ...
                  that.data.canLocation && that.canvas.fillText(`拍摄地址：${that.data.district}`, 0, -40)
                  that.data.canTime && that.canvas.fillText(`拍摄时间：${that.data.currentTime}`, 0, -20)
                  that.data.canProjectName && that.canvas.fillText(`项目名称：${that.data.projectName}`, 0, 0)
                }
              }
